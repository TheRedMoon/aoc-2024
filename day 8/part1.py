...
def create_anti_node_coord_from_pair(pair):
    ...
    anti_node_pair_1_coord = (pair[0][0] + dist_x, pair[0][1] + dist_y)
    anti_node_pair_2_coord = (pair[1][0] - dist_x, pair[1][1] - dist_y)
    
    #print(anti_node_pair_1_coord, anti_node_pair_2_coord)
    return anti_node_pair_1_coord, anti_node_pair_2_coord    
...
def check_if_valid_coordinate(x, y, coord):
    if coord[0] < 0 or coord[0] >= y or coord[1] < 0 or coord[1] >= x:
        return False
    return True
...
if __name__ == '__main__':
    ...
    #anti_node_plus_frequency_grid = create_grid_from_coordinates(x, y, valid_anti_nodes)
